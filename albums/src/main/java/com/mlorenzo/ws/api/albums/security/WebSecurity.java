...
public class WebSecurity extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests((requests) -> 
			// Sólo se permite el acceso a las rutas que coincidan con la expresión "/albums/**" mediante peticiones http de tipo GET a los usuarios autenticados que tengan el scope "profile"
			// Dicho de otra manera, para acceder a las rutas que coincidan con la expresión "/albums/**", las peticiones http tiene que ser de tipo GET y los token JWT que viajan en la cabecera(cabecera "Authorization") de esas peticiones tienen que ser válidos y contener el scope "profile"
			/* requests.antMatchers(HttpMethod.GET, "/albums/**").hasAuthority("SCOPE_profile")*/ // El prefijo "SCOPE_" es necesario indicarlo aquí porque, cuando Spring Security obtiene por debajo los scopes de los token JWT, añade ese prefijo de forma automática a esos scopes
			// Sólo se permite el acceso a las rutas que coincidan con la expresión "/albums/**" mediante peticiones http de tipo GET a los usuarios autenticados que tengan el role "developer"
			// Dicho de otra manera, para acceder a las rutas que coincidan con la expresión "/albums/**", las peticiones http tiene que ser de tipo GET y los token JWT que viajan en la cabecera(cabecera "Authorization") de esas peticiones tienen que ser válidos y contener el role "developer"	
			requests.antMatchers(HttpMethod.GET, "/albums").hasAuthority("ROLE_developer") // Spring Security maneja por debajo los roles que obtiene del token JWT usando el prefijo "ROLE_". Entonces, si usamos el método "hasAuthority" o el método "hasAnyAuthority" para el control de acceso a los recursos, es necesario especificar el prefijo "ROLE_" en esos métodos porque no lo tienen en cuenta
